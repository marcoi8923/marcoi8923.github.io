// Локальное хранилище устройства (IndexedDB): токен, кэш файлов, позже очередь правок (ADR-002, ADR-004).
...
export async function getToken(): Promise<string | null> {
  const v = await (await db()).get('kv', 'token')
  return typeof v === 'string' ? v : null
}

export async function setToken(token: string): Promise<void> {
  await (await db()).put('kv', token, 'token')
}
...
/** «Выйти»: стереть с устройства всё — токен и кэш данных. */
